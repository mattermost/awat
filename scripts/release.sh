...
release-notes --org mattermost --repo awat --start-sha "${LATEST_RELEASE_NEXT_COMMIT_SHA}" --end-sha "${FUTURE_RELEASE_SHA}"  --output ./build/_output/docs/relnote.md --required-author "" --branch master
...
cp ./build/_output/bin/awat ./build/_output/bin/awat-linux-amd64

hub release create -d -F ./build/_output/docs/relnote.md -a ./build/_output/bin/awat-linux-amd64 ${CIRCLE_TAG}
rm -rf ./build/_output/